/*
 * File was taken almost as is from the link below:
 * https://github.com/clusterfarmem/cfm/blob/master/quicksort/quicksort.cpp
 */
...
void print_time_diff(time_point<high_resolution_clock> start,
	...
	auto diff = end - start;
	std::cout << duration<double, std::nano> (diff).count() << "\n";
}

void print_time_diff_ms(time_point<high_resolution_clock> start,
	time_point<high_resolution_clock> end) {
	auto diff = end - start;
	std::cout << duration<double, std::milli> (diff).count() << "\n";
}

...
CGROUP_MAX_MEM_BYTES=$((4096*1024*1024)) # 4GB
sar -B 1 60 > /tmp/${system}_bw.out &
bash -c "sleep 10; echo ${CGROUP_MAX_MEM_BYTES} > /sys/fs/cgroup/memory/${CGROUP_NAME}/memory.limit_in_bytes" &
sudo cgexec -g memory:${CGROUP_NAME} ./page_strided 4000
sleep 4
tail -n +3 /tmp/${system}_bw.out | awk '{print $2,$3,$5}' | tr -s '[:blank:]' ',' > ${res_dir}/bw.csv
